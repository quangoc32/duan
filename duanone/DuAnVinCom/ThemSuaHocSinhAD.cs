...
namespace DuAnVinCom
{
    ...
    public partial class ThemSuaHocSinhAD : Form
    {
            ...
            if (ValidateInputs())
            {
                try
                {
                    ...
                    HocSinh newHocSinh = new HocSinh
                    {
                        ...
                        ChaHS = hoTenCha,
                        MeHS = hoTenMe,
                        MaLop = cbxLop.SelectedValue.ToString(), // Lưu giá trị của cbxLop
                        IsHidden = false,
                    };
        ...
        private void InitializeComboBox()
        {
            ...
            // Load LopHoc data into cbxLop
            var lopHocList = db.LopHocs.Select(lop => new { lop.MaLop, lop.TenLop }).ToList();
            cbxLop.DisplayMember = "TenLop";
            cbxLop.ValueMember = "MaLop";
            cbxLop.DataSource = lopHocList;
        }
        ...
        private void LoadData()
        {
            try
            {
                var studentsWithClass = from hs in db.HocSinhs
                                        join lop in db.LopHocs on hs.MaLop equals lop.MaLop
                                        ...
                                        select new
                                        {
                                            ...
                                            hs.ChaHS,
                                            hs.MeHS,
                                            LopHoc = lop.TenLop // Thêm thông tin lớp học vào dữ liệu
                                        };

                dgvHocSinh.DataSource = studentsWithClass.ToList();
            }
            catch (Exception ex)
            {
                ...
                if (dgvHocSinh.SelectedRows.Count > 0)
                {
                    try
                    {
                        ...
                        if (selectedHS != null)
                        {
                            ...
                            selectedHS.MaLop = cbxLop.SelectedValue.ToString(); // Cập nhật thông tin lớp
            ...
            if (dgvHocSinh.CurrentRow != null)
            {
                try
                {
                    ...
                    if (hs != null)
                    {
                        ...
                        cbxLop.SelectedValue = hs.MaLop; // Sử dụng SelectedValue thay vì SelectedItem
                    }
                }
                catch (Exception ex)
                {
